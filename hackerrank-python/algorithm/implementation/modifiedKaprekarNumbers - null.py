# -*- coding: utf-8 -*-
"""kaprekar numbers at hackerrank.com
"""
"""
Problem Statement
...
low, high = int(raw_input()), int(raw_input())
##print low, high
foundOne = 0
if low <= 1 :
    print 1,
for oneNum in range(low, high+1) :
    sqNum = str(oneNum**2)
##    print sqNum
    middle = len(sqNum)/2
##    print middle, type(middle)
    try :
        left = int(sqNum[:middle])
        right = int(sqNum[middle:])
        if left + right == oneNum :
            foundOne = 1
            print oneNum,
    except :
        continue
